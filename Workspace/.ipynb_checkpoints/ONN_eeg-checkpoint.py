...
class ONN:
    ...
    EPS = 2#, 0.5        # ε - coupling strength for error signal e(t) = D(t) - P(t)
    ...
    FREQ_BANDS = np.array([[1, 3], [4, 7], [8, 12], [13, 30], [30, 100]]) #, [8, 12], [13, 30]
    ...
    def initialize(self):
        ...
        #np.fill_diagonal(self.A, 0) #Few region have connection to themselves! What does this mean?
        ...
        self.J[:] = self.get_J_matrix(self.A)
            ...
            for j in range(self.N_NODES):
                ...
                for k in range(self.N_SUB):
                    if A[i,j] > 0: G.add_edge(i*self.N_SUB + k, 
                                              ...
                                              weight=wt)
                    #TODO: Uncomment when using inter-region connectivity
                    #self.P_VAL[i*self.N_SUB + k, j*self.N_SUB + k] = mb_atlas.P_VAL[i,j]
