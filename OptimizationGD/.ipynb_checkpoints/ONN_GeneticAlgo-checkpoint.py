...
class ONN:
    ...
    def generate_samples(self, NSAMPLES, dt, PHI, OMEGA, A, ALPHA):
        samples = []
        PHI_ = PHI
        ...
        for idx in range(NSAMPLES):
            ...
            samples.append(np.sum(ALPHA*np.cos(PHI_)))
        
        return np.array(samples)[:,None]
    
    def delete_me(self, NSAMPLES, dt, PHI, OMEGA, A, ALPHA):
        samples = []
        phase_diff = []
        phi_dot = []
        PHI_ = PHI
        ...
        for idx in range(NSAMPLES):
            ...
            samples.append(np.sum(ALPHA*np.cos(PHI_)))
            phase_diff.append(NORM_PHASE)
            phi_dot.append(dPHI)
        
        return np.array(samples)[:,None], np.array(phase_diff), np.array(phi_dot)
    ...
    def generate_test_samples(self, NSAMPLES=10000, sampling_rate=1000):
        ...
        return self.generate_samples(NSAMPLES, dt, self.PHI_Test, 
    ...
    def fit(self, data):
        ...
        bounds = [(0.1, 10)] * len(x0)  # bounds for each element of x 
    ...
    def objective(self, updated_weight):
        ...
        #weights = self.A.ravel()
        #mask = weights != 0
        #weights[mask] = updated_weight
        #A_updated = weights.reshape(self.N, self.N)
        
        # Gradient calculation for weight can be parallalized instead of 
        # running objective function serially for each weight
        curSignal = self.generate_samples(self.NSAMPLES, dt, self.PHI, 
                                          self.OMEGA, A_updated, self.ALPHA)
        err = np.sum((self.TestData - curSignal)**2)
        self.ERR.append(err)
        if (len(self.ERR)%100==0):
            print("Evaluating...")
        return err
    
    def my_callback(self, updated_weight):
